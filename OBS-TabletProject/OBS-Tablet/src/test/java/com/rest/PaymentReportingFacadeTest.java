...
public class PaymentReportingFacadeTest {
    ...
    public void testAPIReqests() throws Exception {
        ...
        TestOBSApplication.injectMocks(paymentReportingFacade);
        ...
        setupFacade.login("Asdf1234", new Callback() {
            ...
            public void onTaskCompleted(Object result) {
                ...
                paymentReportingFacade.getPayments(PaymentReportingFacade.PaymentRequestType.RECENT, new Callback<GetPaymentsResponse>() {
                    @Override
                    public void onTaskCompleted(GetPaymentsResponse result) {

                        for (PaymentEntry objs : result.getPayments().subList(0,result.getPayments().size()<10?result.getPayments().size():10)) {
                            Log.d("Recent Payment", "ID: " + objs.getTransactionId() + "Amount: " + objs.getDisplayAmount() + "Date: " + objs.getPaymentDate().toString());
                            paymentReportingFacade.getPaymentDetails(objs, new Callback<PaymentEntry>() {
                                @Override
                                public void onTaskCompleted(PaymentEntry result) {
                                    Log.d("Recent Payment Details", "ID: " + result.getTransactionId() + "Amount: " + result.getDisplayAmount() + "Date: " + result.getPaymentDate().toString() + "Details: " + result.getDetails().getClass());
                ...
                paymentReportingFacade.getPayments(PaymentReportingFacade.PaymentRequestType.FUTURE, new Callback<GetPaymentsResponse>() {
                    @Override
                    public void onTaskCompleted(GetPaymentsResponse result) {

                        for (PaymentEntry objs : result.getPayments()) {
                            Log.d("Future Payment", "ID: " + objs.getTransactionId() + "Amount: " + objs.getDisplayAmount() + "Date: " + objs.getPaymentDate().toString());
                        }
                        Log.d("onTaskCompleted", result.toString());

                        final PaymentEntry paymentEntry = result.getPayments().get(0);
                        paymentReportingFacade.getPaymentDetails(paymentEntry, new Callback<PaymentEntry>() {
                            @Override
                            public void onTaskCompleted(PaymentEntry result) {
                                Log.d("Payment Details: ", ""+ (paymentEntry.getPaymentDate() != null));
                            }
                ...
                paymentReportingFacade.getPayments(PaymentReportingFacade.PaymentRequestType.RECENT, 2013, 11, 18, new Callback<GetPaymentsResponse>() {
                    @Override
                    public void onTaskCompleted(GetPaymentsResponse result) {
                ...
                for (int i = 12; i>5; i--){
                    paymentReportingFacade.getPayments(PaymentReportingFacade.PaymentRequestType.RECENT, 2013, i, new Callback<GetPaymentsResponse>() {
                        @Override
                        public void onTaskCompleted(GetPaymentsResponse result) {
