...
public class LineGraph extends View {
    ...
    public void addtextToLine(int lineIndex, double x, double y){
        addTextToLine(lineIndex, (float) x, (float) y);
    }
    ...
    public void addTextToLine(int lineIndex, float x, float y){
        LinePoint p = new LinePoint(x, y);
        addTextToLine(lineIndex, p);
    }
    ...
    public void addTextToLine(int lineIndex, LinePoint point){
        Line line = getLine(lineIndex);
        line.addPoint(point);
        lines.set(lineIndex, line);
        resetYLimits();
    ...
    public void addTextToLine(int lineIndex, LinePoint[] points){
        Line line = getLine(lineIndex);
        for(LinePoint point : points){
            line.addPoint(point);
        }
		...
		if (fullImage == null || shouldUpdate) {
			...
			for (Line line : lines){
				...
				if (lineCount == lineToFill){
					paint.setColor(Color.parseColor("#26733E"));
					paint.setAlpha(255);
					...
					for (LinePoint p : line.getPoints()){
						...
						if (count == 0){
							...
							Path pa = new Path();
							pa.moveTo(lastXPixels, lastYPixels - graphHeight);
							pa.lineTo(newXPixels, newYPixels - graphHeight);
			...
			for (Line line : lines){
				...
				for (LinePoint p : line.getPoints()){
					...
					if (count == 0){
						...
						newYPixels = getHeight() - bottomPadding - (usableHeight*yPercent);
                        //Draw Lines here.
						canvas.drawLine(lastXPixels, lastYPixels - graphHeight, newXPixels-1, newYPixels - graphHeight, paint);
						lastXPixels = newXPixels;
						lastYPixels = newYPixels;
